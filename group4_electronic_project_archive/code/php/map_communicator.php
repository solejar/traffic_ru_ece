...
//Tested by: Mhammed Alhayek, Ridwan Khan, Shubhra Paradkar, Lauren Williams
...
function get_route($my_location){
    ...
    // check if you need alternative routes
    $alts = $my_location["alternative"];
    ...
    if ($alts == "true"){
        $route_url = "https://maps.googleapis.com/maps/api/directions/json?origin=".$route_start."&destination=".$route_end."&alternatives=true&key=".$loc_api_key;
    }
    else{
...
function parse_route($routeJSON, $routeNum){

    // parsing the json for the part that contains the steps for the directions
    $stepsArr = $routeJSON[$routeNum]->legs[0]->steps;
    // number of steps
    $numSteps = sizeof($stepsArr);

    $geo_keys_arr = array(AIzaSyCJ5LXZxb1ARZdOi_QylcU32ZJ_Bkt3Ur8, AIzaSyAcKHE26DQ2dnpEc0kKHr4dzUXSxMTM3Os, AIzaSyAfH0FJQ7DBaNuddAGxBqYMTFZDHakHp9g);
    ...
    //$geo_api_key = AIzaSyCJ5LXZxb1ARZdOi_QylcU32ZJ_Bkt3Ur8;       // extra api key
    //AIzaSyAcKHE26DQ2dnpEc0kKHr4dzUXSxMTM3Os
    //AIzaSyAfH0FJQ7DBaNuddAGxBqYMTFZDHakHp9g

    $rand_key = rand(0,2);
    $geo_api_key = $geo_keys_arr[0];
...
// parses the json for the distance to get from start to finish for a route
function parse_distance($routeJSON, $routeNum){
    // parsing the json for the part that contains the total distance for the route
    $dist = $routeJSON[$routeNum]->legs[0]->distance->value;
